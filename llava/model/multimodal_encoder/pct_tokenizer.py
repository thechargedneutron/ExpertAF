...
class PCT_Tokenizer(nn.Module):
    ...
    def forward(self, joints, joints_feature=None, cls_logits=None, train=False, return_embeds=True):
        """Forward function. """
        if joints.dtype != self.start_embed.weight.dtype:
            joints = joints.to(self.start_embed.weight.dtype)
        ...
        if train or self.stage_pct == "tokenizer":
            ...
            encodings = torch.zeros(
                encoding_indices.shape[0], self.token_class_num, device=joints.device, dtype=joints.dtype)
        ...
        # encodings is the tokenizer with binary values and part_token_feat is the embedding
        if return_embeds:
    ...
    def decode_tokens(self, encoding_indices, bs, cls_logits=None):
        '''
        encoding_indices dim should be [bs * 34]
        '''
        encodings = torch.zeros(
            encoding_indices.shape[0], self.token_class_num)
        encodings.scatter_(1, encoding_indices.unsqueeze(1), 1)
        part_token_feat = torch.matmul(encodings, self.codebook)
